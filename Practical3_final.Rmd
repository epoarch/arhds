...
**Pneumonia is a respiratory disease that inflames air sacs in the lungs, causing them to fill with fluid. Symptoms are coughing, fever, chills, and difficulty breathing. From the article provided, it is the leading cause of childhood mortality worldwide. Both detection of pneumonia (as opposed to other respiratory disease) and differentiation between viral and bacterial types for treatment can be done using chest x-rays. This makes early identification both impactful on a large number of patients and efficacious in reducing the amount of time spent analyzing X-rays.**
...
```{r}
data_folder = "dataprac3/lab3_chest_xray"
...
**The filenames show both bacterial and viral pneumonia types. This adds another layer of difficult as predicting pneumonia will require not only differentiation from normal chest x-rays, but also differentiation between pneumonia types. There are also no patient IDs for those images where the x-ray was normal, which might be an issue if we want to sort the images by unique patient ID later on.**
...
```{r}
# Exploring dataset
base_dir <- "dataprac3/lab3_chest_xray"
...
**We may need to normalize attributes of the image such as the contrast, patient size, capture model and frequency. Some of these images seem to have different sized patients or are in zoomed out images, while others appear to have poor contrast. We may need to remedy these differences across the dataset prior to training so that the images contain the same objects and are of usable quality.**
...
**In order to balance the dataset, we could augment existing data to add training samples that balance the sets. We could then automate this through use of a generative adversarial network that produces fake samples to be discriminated from real samples.**
...
**The more data are augmented the further they will stray from the original dataset. While this may be desirable in replicating some real-world scenarios and for generalizability more braodly, they may decrease the performance of the model on the original dataset and depending on the modification, may not actually represent real-world artifacts or transformations that will be encountered. If our tool were only meant for high performance on specific trial data, generalization to clinical data may not be helpful. Additionally, as the number of augmentations increase, it may also be difficult to discern which are actually improving performance.**
...
**The 2012 challenge was to train models on a subset a large dataset of hand-labelled images and predict labels for unlabelled images identifying the main objects present. The dataset contains 10,000,000 images and 10,000 object classes. For our purpose, a pre-trained model used for ImageNet such as MobileNet can be useful as a source of transfer learning from which we can improve our model.**
...
**MobileNet models build convolutional neural networks similar to the one we have created in previous steps. However, since MobileNets contain hyper-parameters that choose the appropriate model size for our problem, this will optimize the model for our purposes in terms of latency and accuracy. MobileNets have proven to be effective in similar tasks of object detection and finegrain classsification needed to makes diagnoses from our medical images.**

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?

**This network has 2.5m parameters. Better performing networks like VGG available in torch show marginal improvements in accuracy using ImageNet (71.5%), but contain 138m parameters.**

In the following you can see an example of how we can load pre-trained models in our codes.
#using mobilenet v2 architecture
```{r}
train_X <- array(data = unlist(train_X), dim = c(1000, 224, 224, 3))
test_X <- array(data = unlist(test_X), dim = c(200, 224, 224, 3))
val_X <- array(data = unlist(val_X), dim = c(16, 224, 224, 3))
```
