...
**EEG intensities spike when eyes are opened, then activity steadily decreases to baseline as the brain adjusts to the visual input.**
...
**We would expect the states to be temporally correlated due to the visible differences in EEG intensities at times when eyes are open versus closed.**
...
```{r compare_summary_stats, echo=TRUE}
...
**The F7, O1, and possibly P7 sensors appears to be the only sensors with greater mean and/or median signal intensities.**
...
**Stationarity describes whether the time series has a significant effect from trends that may change the distribution over time, such as seasonality in a longer time series.**
...
**We are interested in whether there is an effect of the eyes being open or closed or a particular sensor where the distribution of the time series is significantly different at different points in time. If there is a significant effect of the eyes being open or closed for example, we may want to employ methods to reduce or eliminate this effect so that the times series is consistent. The results of the stationarity test tell us that there is significant evidence against the null hypothesis of no stationarity and that the data are stationary for the sensor and eye variables.**
...
**Autocorrelation appears to be present in the FC5 and F7 plots and especially for the eye status variable. There also appears to be cross-correlation of the FC5 and F7 fields with eachother.**
...
**The power spectral density for the eyes open shows values closer to 10 dB versus -20 dB for eyes closed. Channels 6 and 14 are consistently above 40 dB in the eyes open spectrum, whereas channels 13, 9, and 1 are around this power in the eyes closed spectrum. Overall, the channels appear to have very different power under the two conditions.**
...
**The ICA shows one source of neuronal activity, the V1 channel. It is difficult to say whether the channel correlates to the eye status variable, since there are no obvious patterns in the channel signal in periods where eyes are open or closed.**

### Eye Opening Prediction

Now that we've explored the data let's use a simple model to see how well we can predict eye status from the EEGs:

```{r xgboostmodel, cache=TRUE}
...
xgbmodel <- xgboost(data = eeg_train_matrix, 
...
# Convert the training and validation datasets to matrices
eeg_train_labelsrf <- eeg_train$eyeDetection
eeg_validate_labelsrf <- eeg_validate$eyeDetection

library(randomForest)
rf_model <- randomForest(eeg_train$eyeDetection~., data = eeg_train_matrix, label = eeg_train_labelsrf)
...
```{r test}
validate_rf <- predict(rf_model, newdata = eeg_validate_matrix)

#confusion matrix rf
rf_confusion_matrix <- confusionMatrix(factor(validate_rf, levels = c(0, 1)),
...
```{r}
validate_xgb <- predict(xgbmodel, newdata = eeg_validate_matrix)
validate_xgb <- ifelse(validate_xgb > 0.5, 1, 0)
#confusion matrix xgb
xgb_confusion_matrix <- confusionMatrix(factor(validate_xgb, levels = c(0, 1)),
                                    factor(eeg_validate_labels, levels = c(0, 1)))

print(xgb_confusion_matrix)
```

**Accuracy of rf = 92% > xgb = 83%, now we will use it on the test set**

```{r}
test_rf <- predict(rf_model, newdata = eeg_test)

confusion_matrixtest <- confusionMatrix(factor(test_rf, levels = c(0, 1)),
                                    factor(eeg_validate_labels, levels = c(0, 1)))

print(confusion_matrixtest)
```

**Accuracy of the rf model on the test set = 0.86 (95% CI: 0.8424, 0.8679)**
...
**We could try using a Hidden Markov Model as a state-space model in which the eye status would be the hidden value derived from the observed inputs (EEG data). We could also try using an ARIMA model to predict eye status from the EEG data from past values (Autoregression), past errors (moving average), and differencing of EEG data.**
...
**HMM can be loaded through CRAN**
...
**Some of the lecture content felt advanced given my very limited experience with machine learning and I also had a difficult time with knowing how to code new models and their arguments to make them run properly. The tutorials were very interesting and relevant to course material. Critically appraising research that felt advanced on the surface helped my understanding of modelling techniques and limitations of some approaches and study designs.**
 ...
 **yes, useful to have the code in the future for projects, especially not having the same experience with learning new languages as a computer science student.**
 
- What would you add or remove from the course? 

- What was the main thing you will take away from this course?

**Where machine learning can be applied to health data, which methods are appropriate, limitations, and when to use traditional vs ML approaches.**