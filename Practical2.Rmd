...
* note_id: identification number for each unique note  
* description: description of the patient encounter  
* medical_specialty: specialist(s) seeing to the patient  
* sample_name: procedures and diagnoses relevant to encounter  
* transcription: unstructured text from EMR  
* keywords: semi-structured text summarizing appointment  
...
**There are 40 different medical specialties featured.**
...
**Stop words and numbers removed, there are the most bigrams in surgery, followed by orthopedic and radiology.**
...
tokenized.data.sentences <- analysis.data %>% tidytext::unnest_tokens(
  sentence, 
  transcription, 
  token = "sentences",
  to_lower=TRUE)

tokenized.data.sentences %>% dplyr::group_by(medical_specialty) %>% dplyr::distinct(sentence) %>% dplyr::summarise(n=dplyr::n())

```

**The number of unique sentences follows the same pattern, with the most in surgery followed by orthopedic and radiology.**
...
**Will likely not work well across all specialties as each specialty has important differences in the usage of medical terminology and lemmatizing may work better within those specialties to ensure that important words are being lemmatized correctly. Terms with similar stems may not have similar meanings due to their affixes or due to differences in use in different specialties.**
...
**There is more similarity in lemma frequency between radiology and orthopedic than surgery and orthopedic. This is opposite to what I would expect based orthopedic specialists often being orthopedic surgeons. Based on these surgeons practicing a similar specialty to general surgery, I would expect more overlap in the lemmas. However, perhaps the high usage of medical imaging in both radiology and orthopedic is the reason for the similarities in lemma frequency.**
...
```{r}
lemma.freq.surg <- lemmatized.data %>% 
  dplyr::count(medical_specialty, lemma) %>%
  dplyr::group_by(medical_specialty) %>% 
  dplyr::mutate(proportion = n / sum(n)) %>%
  tidyr::pivot_wider(names_from = medical_specialty, values_from = proportion) %>%
...
ggplot2::ggplot(lemma.freq.surg, ggplot2::aes(x=proportion, 
                                         y=`Surgery`,
...
```
**Slightly more similarity in lemma frequency between surgery and radiology than surgery and orthopedic.**
...
**Evoked response seems to be a strange lemma to be used so frequently in radiology. Perhaps this is part of a standard EMR form. Ejection fraction also seems to be a lemma that shows up a relatively high number of times but when using invariant document frequency is not frequently used, indicating its disproportionate use in some documents.**
...
```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(stringr::str_detect(transcription, 'ejection fraction')) %>% dplyr::slice(1)
```
**The transcription shows use of the lemma 'ejection fraction' in the context of an abnormal heart rate that required a valvular heart disease procedure with implantation of a pacemaker. Ejection fraction would be the percentage of blood leaving the heart during ejection and so would be a reasonable part of this procedure.**
...
**The top terms are slightly different when using a 6-topic LDA versus the 5-topic LDA. The specialties appear to assign less to a single subtopic when going from 5- to 6-topics. This may indicate the additional subtopic is similar to the other topics in the 5-topic LDA and that each specialty uses a diverse mix of these topics rather fitting into a clearly defined subtopic. Generally, there seems to be a high degree of overlap of terms in the subtopics and a diverse mixture of these subtopics within each specialty.**
